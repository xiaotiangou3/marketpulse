...
def route_user_intent(user_prompt: str, has_uploaded_file: bool, file_context: str = None, holdings_str: str = None, status_callback: Optional[Callable[[str, Optional[str]], None]] = None) -> RouterOutput:
    ...
    system_instruction = (
        ...
        "- If the user asks to analyze, discuss, evaluate, or review qualitative strategy guidelines or rules from an uploaded file, do NOT route to 'backtest'. The 'backtest' action is strictly for quantitative technical indicator calculations (RSI, SMA, EMA, MACD, Bollinger Bands) on stock price data. For qualitative strategy discussions, reviews, or questions, output 'none' as action_type so the conversational agent can directly discuss the strategies in detail.\n\n"
...
def generate_conversational_response(user_prompt: str, strategies_str: str, file_context: str = None, status_callback: Optional[Callable[[str, Optional[str]], None]] = None) -> str:
    ...
    system_instruction = (
        ...
        "If the file context contains qualitative strategy rules or IPS guidelines, analyze each rule in detail, explain whether they are useful, and provide a clear, accurate conclusion.\n\n"
